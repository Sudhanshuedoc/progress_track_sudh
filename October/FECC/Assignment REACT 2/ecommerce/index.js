...
sort.addEventListener("change", function () {
  ...
  if (sortval === "low_to_high") {
    products.sort((a, b) => {
      return a.price - b.price;
    });
    displayCard(products);
  } else if (sortval === "high_to_low") {
    products.sort((a, b) => {
...
filter.addEventListener("change", function () {
  ...
  let val = products.filter((ele) => {
    return !category || ele.category === category;
  });
  displayCard(val);
});
function displayCard(product) {
  root.innerHTML = "";
  product.map((ele) => {
    let card = cardMaker(ele);
    root.append(card);
  });
}
function cardMaker(prod) {
  ...
  rating.textContent = "RATING: " + prod.rating.rate;
